...
module.exports = {
  ...
  index: async (req, res) => {
    ...
    if (DecodeRole(role, 10) === 0) {
      req.query._id ? dataResponse = await Friend.find({'_id': req.query._id}).lean() : req.query._fbId ? dataResponse = await Friend.find({'_facebook':req.query._fbId,'_account': userId}).lean() : req.query._size ? dataResponse = (await Friend.find({'_account': userId}).lean()).slice(0, Number(req.query._size)) : req.query._size && req.query._page ? dataResponse = (await Friend.find({'_account': userId}).lean()).slice((Number(req.query._page)-1)*Number(req.query._size), Number(req.query._size)*Number(req.query._page)) : dataResponse = await Friend.find({'_account': userId}).lean()
