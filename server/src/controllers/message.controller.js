  ...
  index: async (req, res) => {
    ...
    if (DecodeRole(role, 10) === 0) {
      ...
      await req.query._id ? dataResponse = await Message.find({
        ...
        path: '_sender',
        select: '-cookie'
      }) : dataResponse = await Message.find({'_account': userId}).populate({
        path: '_receiver',
        select: '-_account -_facebook'
      }).populate({path: '_sender', select: '-cookie'})
