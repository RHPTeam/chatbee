          ...
          <tbody>
            <tr v-for="(v, i) in 6" :key="i">
              <td v-for="(v, j) in 7" :key="j" :class="date[i * 7 + j] && date[i * 7 + j].status" :date="date[i * 7 + j] && date[i * 7 + j].date"
                ...
                <div class="day">{{ date[i * 7 + j] && date[i * 7 + j].text }}
                </div>
                <div v-for="item in dates" :key="item.id" v-if="date[i * 7 + j] && date[i * 7 + j].time.getFullYear()==item.time_at.getFullYear()
                  &&date[i * 7 + j] && date[i * 7 + j].time.getMonth()==item.time_at.getMonth()
                  &&date[i * 7 + j] && date[i * 7 + j].time.getDate()==item.time_at.getDate()" class="item">
                  {{
          ...
          <form v-if="show" class="add_date">
            ...
            <div class="form_group">
              <table class="table table_striped">
                ...
                <tbody>
                   <tr v-for="item in dates" :key="item.id" v-if="stringify(item.time_at)==pickedValue" class="add_item">
                 
                    <th scope="row">1</th>
                    <td>{{formatAMPM(item.time_at)}}</td>
                    <td>{{item.title}}</td>
                    <td>Bài viết abc </td>
                    <td> <span class="edit" @click ="edit(item)">Sửa </span> || <span class="delete" @click ="remove(item.id)"> Xóa </span></td>
                  </tr>
            ...
            <div class="form_group">
              ...
              <input type="text" v-model="item.title" class="form_control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nhập tiêu đề">
            </div>
            <div class="btn_group mr_2" role="group" aria-label="Third group">
              <button type="button" @click="action()" class="btn btn_success">{{acctionStt?"Thêm":"Sửa"}}</button>
            </div>
            <div class="btn_group" role="group" aria-label="Third group">
              <button type="reset" @click="hide()" class="btn btn_secondary">Thoát</button>
            </div>
